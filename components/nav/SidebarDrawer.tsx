...
export function SidebarDrawer({
  ...
  // navega a otra pantalla (los Link del menú solo cierran el drawer, no
  // tocan este estado), el aviso no debe reaparecer ahí. Ajustado durante el
  // render, no en un efecto — ver el mismo patrón en Sidebar.tsx.
  ...
  return (
    ...
    >
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-72 max-w-[85vw] flex-col border-r border-border/80 bg-card/98 backdrop-blur-2xl rounded-r-[2rem] shadow-2xl overflow-hidden transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border/60 px-4 py-3.5 bg-muted/20">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="relative grid h-8 w-8 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-accent to-orange-600 text-white shadow-sm">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              type="button"
              onClick={onCerrar}
              className="grid h-8 w-8 place-items-center rounded-2xl text-muted-foreground hover:bg-muted hover:text-foreground active:scale-95 transition-all"
        ...
        <div className="grid grid-cols-2 gap-2 p-3 border-b border-border/40">
          <Link
            href="/ot/nueva"
            onClick={onCerrar}
            className="flex min-h-10 items-center justify-center gap-1.5 rounded-2xl bg-accent px-2 text-xs font-bold text-white shadow-sm transition-all hover:brightness-105 active:scale-95"
          >
        ...
        {/* 3. Lista de Rutas */}
        <nav className="flex-1 space-y-0.5 overflow-y-auto px-2.5 py-3 overscroll-contain">
          ...
          {itemsVisibles(ITEMS_NAV, rol, vistasPermitidas).map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={onCerrar}
                className={`group relative flex min-h-11 items-center gap-3 rounded-2xl px-3 text-xs font-bold transition-all active:scale-[0.98] ${
                  activo
              ...
              >
                ...
                <Icono
                  className={`h-4 w-4 shrink-0 transition-transform group-hover:scale-110 ${activo ? "text-accent" : "text-muted-foreground"}`}
        ...
        <div className="border-t border-border/60 bg-muted/20 p-3">
          {confirmandoLogout ? (
            <div className="rounded-2xl border border-destructive/30 bg-destructive/10 p-2.5 text-center space-y-2 animate-in fade-in">
              <p className="text-xs font-bold text-destructive">¿Cerrar sesión?</p>
              <div className="flex items-center gap-1.5">
                <form action={cerrarSesion} className="flex-1">
                  <button
                    type="submit"
                    className="flex min-h-8 w-full items-center justify-center gap-1 rounded-xl bg-destructive text-xs font-bold text-white shadow-xs hover:brightness-105 active:scale-95"
                  >
                ...
                <button
                  ...
                  className="flex min-h-8 flex-1 items-center justify-center gap-1 rounded-xl border border-border bg-card text-xs font-semibold text-foreground hover:bg-muted active:scale-95"
                >
            ...
            <div className="flex items-center justify-between gap-2 rounded-2xl border border-border/60 bg-card/80 px-2.5 py-2">
              <div className="flex items-center gap-2 min-w-0">
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-xl bg-zinc-800 text-xs font-bold text-white border border-white/10">
              ...
              <button
                ...
                className="grid h-7 w-7 shrink-0 place-items-center rounded-xl text-muted-foreground hover:bg-destructive/10 hover:text-destructive active:scale-95 transition-all"
              >
